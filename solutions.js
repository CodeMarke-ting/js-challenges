...
// Create bigJS like Math methods
Number.prototype.add = function (num = 0) {
  if (typeof num === "number") {
    return this + num;
  } else {
    throw Error("Provide number");
  }
};

Number.prototype.substract = function (num = 0) {
  if (typeof num === "number") {
    return this - num;
  } else {
    throw Error("Provide number");
...
Number.prototype.multiply = function (num = 1) {
  if (typeof num === "number") {
    return this * num;
  } else {
    throw Error("Provide number");
...
console.log((10).add(10).substract(5).divide(2).multiply(4));
...
arrayLikeObject.push("hello");
arrayLikeObject.push("Rustam");
...
const arr = [23, 3, 34, 45, 2, 1];

const sortedNum = supplier.sort((a, b) => a.id - b.id);

console.log(sortedNum);

const sortedStr = supplier.sort((a, b) =>
...
// custom reduce map and filter
Array.prototype.myMap = function (callback) {
  const newArr = [];
  for (let index = 0; index < this.length; index++) {
    newArr.push(callback(index, this[index], this));
  }
  return newArr;
};
...
Array.prototype.myFilter = function (callback) {
  ...
  for (let index = 0; index < this.length; index++) {
    if (callback(index, this[index], this) === true) {
